...
from typing import *
from entities.Embedder import *
from entities.Matcher import *
...
class EntityMatcher():
  ...
  def configureKnn(self,k_neighbors: int=5, seed: int=42, metric: str='cosine', dim: int=300):
    if self.matcherType != 'knn':
      raise Exception("You can only configure KNN if the matcher type is KNN.")
  ...
  def pipeline(self):
    ...
    embeddings = [self.embedder.getEmbeddings(text, self.removeStopwords) for text in self.singleTable[DATABASE_TEXT_COLUMN_NAME]]
